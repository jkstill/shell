...
noSuffix1="$( basename "$myFile" | sed 's/\(.*\)\..*/\1/' )"
noSuffix2="$( basename "$myFile" | sed 's/\(.*\)\..*/\1/' | sed 's/\(.*\)\..*/\1/')"

suffix1="$( basename "$myFile" | sed 's/.*\.\(.*\)/\1/' )"

suffix2="$( basename "$myFile" | awk -F. '{print $(NF-1)"."$NF}' )"
